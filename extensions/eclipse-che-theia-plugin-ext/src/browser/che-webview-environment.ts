...
export class CheWebviewEnvironment extends WebviewEnvironment {
    ...
    protected async init(): Promise<void> {
        try {
            ...
            const ideServer = await this.cheApi.findUniqueServerByAttribute(SERVER_TYPE_ATTR, SERVER_WEBVIEWS_ATTR_VALUE);
            let domain;
            if (ideServer && ideServer.url) {
                domain = getUrlDomain(ideServer.url);
            }
            const hostName = webviewExternalEndpointPattern && webviewExternalEndpointPattern.value || domain || WebviewExternalEndpoint.pattern;
