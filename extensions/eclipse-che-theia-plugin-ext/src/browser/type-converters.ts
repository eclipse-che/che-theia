...
export function toDefinitionLink(locationLink: model.LocationLink): theia.DefinitionLink {
  return <theia.DefinitionLink>{
    ...
    originSelectionRange: locationLink.originSelectionRange
      ? Converter.toRange(locationLink.originSelectionRange)
      : undefined,
    targetSelectionRange: locationLink.targetSelectionRange
      ? Converter.toRange(locationLink.targetSelectionRange)
      : undefined,
  };
}
...
export function toDocumentHighlightKind(kind: model.DocumentHighlightKind | undefined): types.DocumentHighlightKind {
  switch (kind) {
    case model.DocumentHighlightKind.Text:
      return types.DocumentHighlightKind.Text;
    case model.DocumentHighlightKind.Read:
      return types.DocumentHighlightKind.Read;
    case model.DocumentHighlightKind.Write:
      return types.DocumentHighlightKind.Write;
  }
  return types.DocumentHighlightKind.Text;
}
...
export function toDiagnosticSeverity(severity: types.MarkerSeverity): types.DiagnosticSeverity {
  switch (severity) {
    case types.MarkerSeverity.Error:
      return types.DiagnosticSeverity.Error;
    case types.MarkerSeverity.Warning:
      return types.DiagnosticSeverity.Warning;
    case types.MarkerSeverity.Info:
      return types.DiagnosticSeverity.Information;
    case types.MarkerSeverity.Hint:
      return types.DiagnosticSeverity.Hint;
  }
}

export function toDiagnosticRelatedInformation(
  relatedInformation: model.RelatedInformation[] | undefined
): theia.DiagnosticRelatedInformation[] | undefined {
...
function isLocationLinkArray(array: any): array is model.LocationLink[] {
  return (
    Array.isArray(array) &&
    array.length > 0 &&
    array[0].hasOwnProperty('originSelectionRange') &&
    array[0].hasOwnProperty('targetSelectionRange')
  );
}
