...
export class CdnHtmlTemplate {
    ...
    constructor(readonly htmlWebpackPlugin: any, readonly compilation: any) {
        ...
        for (const key in htmlWebpackPlugin.files.js) {
            const url: string = htmlWebpackPlugin.files.js[key].entry;
        ...
        if (cdnPrefix || monacoCdnPrefix) {
            const monacoFiles: decls.CdnExternal[] = monacoRequirePaths
                .map(elem =>
                    ['.js', '.css', '.nls.js']
                        .map(extension => ({
            ...
            fs.writeFileSync(
                'lib/cdn.json',
                JSON.stringify(
                    (<decls.CdnArtifact[]>cachedChunks)
                        .concat(<decls.CdnArtifact[]>monacoFiles)
                        .concat(<decls.CdnArtifact[]>cachedResourceFiles),
                    undefined,
                    2
                )
            );
