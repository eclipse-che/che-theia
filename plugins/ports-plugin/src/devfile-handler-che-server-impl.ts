...
export class CheServerDevfileHandlerImpl implements DevfileHandler {
    async getEndpoints(): Promise<Array<Endpoint>> {
        ...
        return endpoints.filter((endpoint: Endpoint, index: number, arrayEndpoints: Endpoint[]) => arrayEndpoints.findIndex(indexedEndpoint =>
            (indexedEndpoint.name === endpoint.name && indexedEndpoint.targetPort === endpoint.targetPort && indexedEndpoint.exposure === endpoint.exposure)) === index);
    }
}
